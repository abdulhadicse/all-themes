...
/**
 * Add check feature post meta box
 */
if( ! function_exists( 'philosopy_feture_post_meta_field' ) ) {
    function philosopy_feture_post_meta_field() {
        /**
         * Register Meta Box [check-feature-post]
         */
        add_meta_box( 'check-feature-post', __( 'Is Feature Post ?', 'philosopy'), 'philosopy_is_feture_post_cb', 'post', 'side', 'high' );
    }
}

if( ! function_exists( 'philosopy_is_feture_post_cb' ) ) {
    /**
     * Meta box display callbacks
     *
     * @param Obj $post Single Post Object
     * @return void
     */
    function philosopy_is_feture_post_cb( $post ) {
        //get feture post value
        $is_feture_post = get_post_meta( $post->ID , 'philosopy_is_feture_post', true);
        //check checkbox box value
        $is_feture_post = ( $is_feture_post == '1' ) ? 'checked' : '' ;
        
        ?>
           <input 
               type="checkbox" 
               id="check_feture_post" 
               name="check_feture_post" 
               value="1"
               <?php esc_attr_e( $is_feture_post ); ?>
           >
           <label for="check_feture_post">Feture Post</label>   
       <?php
       
    }
}
add_action( 'add_meta_boxes', 'philosopy_feture_post_meta_field' );

if( ! function_exists( 'philosopy_save_future_post_cb' ) ) {
    /**
     ...
     * @return void
     */
    function philosopy_save_future_post_cb( $post_id ) {
       ...
       if ( isset( $_POST['post_type'] ) && 'post' === $_POST['post_type'] ) {
           //set check feture post field
           $check_feture_post = isset( $_POST[ 'check_feture_post' ] ) ? sanitize_text_field( $_POST[ 'check_feture_post' ] ) : '';
           //update value
           update_post_meta( $post_id, 'philosopy_is_feture_post' ,  $check_feture_post );
           }
       }
}
add_action( 'save_post', 'philosopy_save_future_post_cb' );
